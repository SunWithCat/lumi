...
class MemoryCompactor {
  ...
  List<MemoryMergeResult> findMergeCandidates(List<MemoryItem> memories) {
    ...
    for (var i = 0; i < memories.length; i++) {
      ...
      if (similar.length > 1) {
        ...
        results.add(
          MemoryMergeResult(keep: items.first, remove: items.skip(1).toList()),
        );
      }
  ...
  double calculateDecayedImportance(
    ...
    final decayedImportance =
        originalImportance * _pow(decayFactor, daysSinceAccess);
...
class MemoryMergeResult {
  ...
  const MemoryMergeResult({required this.keep, required this.remove});
}
